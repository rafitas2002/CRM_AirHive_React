-- Reweight seller forecast reliability using full forecast timelines.
-- Goals:
...
-- 3) Keep race reliability tied to monthly forecast reliability from real closed-won outcomes.
...
BEGIN
    ...
    probability_weighted AS (
        SELECT
            ...
            POWER(ROW_NUMBER() OVER (PARTITION BY po.lead_id ORDER BY po.observed_at ASC, po.source_rank ASC)::NUMERIC, 1.35) AS temporal_weight
    ...
    IF COALESCE(v_prob_n, 0) > 0 THEN
        v_prob_effective_n := COALESCE(v_prob_lead_n, 0) + (GREATEST(COALESCE(v_prob_n, 0) - COALESCE(v_prob_lead_n, 0), 0) * 0.35);
        v_prob_raw_acc := 1 - COALESCE(v_prob_brier_avg, 0);
        v_prob_score := GREATEST(
            0,
            LEAST(
                100,
                ROUND((v_prob_raw_acc * (v_prob_effective_n / NULLIF(v_prob_effective_n + 6, 0))) * 100, 2)
            )
        );
    END IF;
    ...
    monthly_weighted AS (
        SELECT
            ...
            mo.forecast_value,
            POWER(ROW_NUMBER() OVER (PARTITION BY mo.lead_id ORDER BY mo.observed_at ASC, mo.source_rank ASC)::NUMERIC, 1.35) AS temporal_weight
        FROM monthly_observations mo
    )
    SELECT
        COUNT(*)::INTEGER,
        COUNT(DISTINCT mw.lead_id)::INTEGER,
        SUM(mw.temporal_weight * GREATEST(0, LEAST(100, 100 - LEAST(100, (ABS(mw.monthly_real_value - mw.forecast_value) / NULLIF(mw.forecast_value, 0)) * 100.0)))) / NULLIF(SUM(mw.temporal_weight), 0),
        SUM(mw.temporal_weight * (((mw.monthly_real_value - mw.forecast_value) / NULLIF(mw.forecast_value, 0)) * 100.0)) / NULLIF(SUM(mw.temporal_weight), 0),
        SUM(mw.temporal_weight * ((ABS(mw.monthly_real_value - mw.forecast_value) / NULLIF(mw.forecast_value, 0)) * 100.0)) / NULLIF(SUM(mw.temporal_weight), 0)
    ...
    IF COALESCE(v_value_n, 0) > 0 THEN
        v_value_effective_n := COALESCE(v_value_lead_n, 0) + (GREATEST(COALESCE(v_value_n, 0) - COALESCE(v_value_lead_n, 0), 0) * 0.35);
        v_value_score := COALESCE(ROUND(v_value_score_avg * (v_value_effective_n / NULLIF(v_value_effective_n + 6, 0)), 2), 0);
    ELSE
        v_value_score := 0;
    END IF;
    ...
    implementation_weighted AS (
        SELECT
            ...
            io.forecast_value,
            POWER(ROW_NUMBER() OVER (PARTITION BY io.lead_id ORDER BY io.observed_at ASC, io.source_rank ASC)::NUMERIC, 1.35) AS temporal_weight
        FROM implementation_observations io
    )
    SELECT
        COUNT(*)::INTEGER,
        COUNT(DISTINCT iw.lead_id)::INTEGER,
        SUM(iw.temporal_weight * GREATEST(0, LEAST(100, 100 - LEAST(100, (ABS(iw.implementation_real_value - iw.forecast_value) / NULLIF(iw.forecast_value, 0)) * 100.0)))) / NULLIF(SUM(iw.temporal_weight), 0),
        SUM(iw.temporal_weight * (((iw.implementation_real_value - iw.forecast_value) / NULLIF(iw.forecast_value, 0)) * 100.0)) / NULLIF(SUM(iw.temporal_weight), 0),
        SUM(iw.temporal_weight * ((ABS(iw.implementation_real_value - iw.forecast_value) / NULLIF(iw.forecast_value, 0)) * 100.0)) / NULLIF(SUM(iw.temporal_weight), 0)
    ...
    IF COALESCE(v_impl_n, 0) > 0 THEN
        v_impl_effective_n := COALESCE(v_impl_lead_n, 0) + (GREATEST(COALESCE(v_impl_n, 0) - COALESCE(v_impl_lead_n, 0), 0) * 0.35);
        v_impl_score := COALESCE(ROUND(v_impl_score_avg * (v_impl_effective_n / NULLIF(v_impl_effective_n + 6, 0)), 2), 0);
    ELSE
        v_impl_score := 0;
    END IF;
    ...
    close_date_weighted AS (
        SELECT
            ...
            POWER(ROW_NUMBER() OVER (PARTITION BY cdo.lead_id ORDER BY cdo.observed_at ASC, cdo.source_rank ASC)::NUMERIC, 1.35) AS temporal_weight,
    ...
    IF COALESCE(v_date_n, 0) > 0 THEN
        v_date_effective_n := COALESCE(v_date_lead_n, 0) + (GREATEST(COALESCE(v_date_n, 0) - COALESCE(v_date_lead_n, 0), 0) * 0.35);
        v_date_score := COALESCE(ROUND(v_date_score_avg * (v_date_effective_n / NULLIF(v_date_effective_n + 6, 0)), 2), 0);
    ELSE
        v_date_score := 0;
    END IF;
    ...
    VALUES (
        ...
        'forecast_calibration_v3_temporal_weighted_snapshots',
        NOW(),
        NOW()
    )
